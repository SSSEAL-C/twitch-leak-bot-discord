...
async def main_parser(streamer_id, display_name, ctx, data2019, data2020, data2021):
    ...
    onlyfiles = [f for f in listdir(os.path.dirname(__file__)+'\\data\\') if isfile(join(os.path.dirname(__file__)+'\\data\\', f))]
    ...
    for file in onlyfiles:
        ...
        month = month[:-4]
        try:
            with open(os.path.dirname(__file__)+'\\data\\'+str(file), 'r') as g:
                reader = csv.reader(g)
                detected = 0
