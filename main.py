...
async def sendmsg(ctx, display_name, amount_count, countfiles):
    await ctx.edit(content=':white_check_mark: `' + display_name + ' data found! (' + str(amount_count) + '/' + str(
        countfiles) + ')`')


async def senderr(ctx, display_name, amount_count, countfiles):
    await ctx.edit(
        content=':x: `' + display_name + ' no data found! (' + str(amount_count) + '/' + str(countfiles) + ')`')
                ...
                for row in reader:
                    if row[0] == id:
                        ...
                        total = float(ad_share_gross) + float(sub_share_gross) + float(bits_share_gross) + float(
                            prime_sub_share_gross) + float(bit_share_ad_gross) + float(fuel_rev_gross) + float(
                            bb_rev_gross)
                        grosstotal = grosstotal + float(total)
                        adtotal = float(ad_share_gross) + adtotal
                        subtotal = subtotal + float(sub_share_gross)
                        bitstotal = bitstotal + float(bits_share_gross) + float(bits_developer_share_gross) + float(
                            bits_extension_share_gross)
                        primestotal = primestotal + float(prime_sub_share_gross)
                        yeardata = [total, ad_share_gross, sub_share_gross, bits, prime_sub_share_gross, str(month),
                                    str(year)]
                        data = [ad_share_gross, sub_share_gross, bits_share_gross, bits_developer_share_gross,
                                bits_extension_share_gross, prime_sub_share_gross, bit_share_ad_gross, fuel_rev_gross,
                                bb_rev_gross, report_date, str(total), str(year)]
                        if year == "19":
...
labels = ['08/19', '09/19', '10/19', '11/19', '12/19', '01/20', '02/20', '03/20', '04/20', '05/20', '06/20', '07/20',
          '08/20', '09/20', '10/20', '11/20', '12/20', '01/21', '02/21', '03/21', '04/21', '05/21', '06/21', '07/21',
          '08/21', '09/21', '10/21']
...
async def revenue(ctx, username: str, *graph: str):
    ...
    if checkid == True:
        ...
        if data2019 != []:
            ...
            data19 = ':moneybag: Gross Total: ' + "`$" + str(
                "{:,}".format(round(total2019, 2))) + " USD`\n" + ':tv: Ad Total: ' + "`$" + str(
                "{:,}".format(round(ad2019, 2))) + " USD`\n" + ':star: Sub Total: ' + "`$" + str(
                "{:,}".format(round(sub2019, 2))) + " USD`\n" + ':stars: Primers Total: ' + "`$" + str(
                "{:,}".format(round(prime2019, 2))) + " USD`\n" + ':ice_cube: Bits Total: ' + "`$" + str(
                "{:,}".format(round(bits2019, 2))) + " USD`"
        if data2020 == []:
        ...
        if data2020 != []:
            ...
            data20 = ':moneybag: Gross Total: ' + "`$" + str(
                "{:,}".format(round(total2020, 2))) + " USD`\n" + ':tv: Ad Total: ' + "`$" + str(
                "{:,}".format(round(ad2020, 2))) + " USD`\n" + ':star: Sub Total: ' + "`$" + str(
                "{:,}".format(round(sub2020, 2))) + " USD`\n" + ':stars: Primers Total: ' + "`$" + str(
                "{:,}".format(round(prime2020, 2))) + " USD`\n" + ':ice_cube: Bits Total: ' + "`$" + str(
                "{:,}".format(round(bits2020, 2))) + " USD`"
        if data2021 == []:
        ...
        if data2021 != []:
            ...
            data21 = ':moneybag: Gross Total: ' + "`$" + str(
                "{:,}".format(round(total2021, 2))) + " USD`\n" + ':tv: Ad Total: ' + "`$" + str(
                "{:,}".format(round(ad2021, 2))) + " USD`\n" + ':star: Sub Total: ' + "`$" + str(
                "{:,}".format(round(sub2021, 2))) + " USD`\n" + ':stars: Primers Total: ' + "`$" + str(
                "{:,}".format(round(prime2021, 2))) + " USD`\n" + ':ice_cube: Bits Total: ' + "`$" + str(
                "{:,}".format(round(bits2021, 2))) + " USD`"
        import pyshorteners

        s = pyshorteners.Shortener()
        timelineurl = "{options:{title:{display:true,text:'Twitch Creator Data - " + display_name + "', fontSize:20}},type:'line',data:{labels:" + str(
        ...
        bargraphurl = "{options:{title:{display:true,text:'Twitch Creator Data - " + display_name + "', fontSize:20}},type:'bar',data:{labels:" + str(
            labels) + ", datasets:[{label:'Gross Total', data: " + str(
            totaldatagross) + ", fill:false,borderColor:'blue'},{label:'Gross Ads', data: " + str(
            totaldataad) + ", fill:false,borderColor:'green'},{label:'Gross Subs', data: " + str(
            totaldatasub) + ", fill:false,borderColor:'yellow'},{label:'Gross Prime', data: " + str(
            totaldataprime) + ", fill:false,borderColor:'red'},{label:'Gross Bits', data: " + str(
            totaldatabits) + ", fill:false,borderColor:'orange'}]}}"
        embed = discord.Embed(title='Twitch Creator Info - ' + display_name, timestamp=time)
        maindata = ':moneybag: Gross Total: ' + "`$" + str(
            "{:,}".format(round(thedata[0], 2))) + " USD`\n" + ':tv: Ad Total: ' + "`$" + str(
            "{:,}".format(round(thedata[1], 2))) + " USD`\n" + ':star: Sub Total: ' + "`$" + str(
            "{:,}".format(round(thedata[2], 2))) + " USD`\n" + ':stars: Primers Total: ' + "`$" + str(
            "{:,}".format(round(thedata[3], 2))) + " USD`\n" + ':ice_cube: Bits Total: ' + "`$" + str(
            "{:,}".format(round(thedata[4], 2))) + " USD`"
        embed.set_thumbnail(url=logo)
        embed.add_field(name=':alarm_clock: Created At', value="`" + str(created) + "`", inline=False)
        embed.add_field(name=':name_badge: Bio', value='```\n' + str(bio) + '\n```', inline=False)
        embed.add_field(name="Data - Total `" + str(month) + '/' + str(year) + "` - `10/21`", value=maindata,
                        inline=False)
        ...
        if graph == "yes" or graph == ():
            embed.add_field(name="Graphs", value="Gross Total: " + s.tinyurl.short(
                'https://quickchart.io/chart?c=' + quote(timelineurl)) + '\nAll Values: ' + s.tinyurl.short(
                'https://quickchart.io/chart?c=' + quote(bargraphurl)))
    ...
    if checkid == False or checkid != True:
        time = datetime.datetime.utcnow()
        embed = discord.Embed(title='Twitch Creator Info - ' + display_name,
...
async def info(ctx):
    ...
    embed.add_field(name=':gear: Command Format',
                    value='`tw!revenue [twitch display_name] <graph? (yes/no)>` (eg. `tw!revenue ludwig yes`)',
                    inline=False)
    embed.add_field(name=':keyboard: Github', value='`https://github.com/SSSEAL-C/twitch-leak-bot-discord`',
                    inline=False)
    embed.set_footer(text="Made by SSSEAL-C")
