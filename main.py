...
async def revenue_yearly_data_split(data2019):
    ...
    return [total2019, ad2019, sub2019, prime2019, bits2019, totaldatagross, totaldataad, totaldatasub, totaldatabits,
            totaldataprime]
    ...
    try:
        ...
        if total2021 == 0:
            ...
            data21 = ':moneybag: Gross Total: ' + "`$" + str(
                ...
                "{:,}".format(bits2021)) + " USD`"
        return [data19, data20, data21, totaldatagross, totaldataad, totaldatasub, totaldatabits, totaldataprime, month,
                year, now]
...
async def store_user_in_database(username):
    if USE_DATABASE:
        ...
        user_dict_for_tinydb = cached_results[username]
        user_dict_for_tinydb['username'] = username
        TinyDB(DATABASE_FILE).table('users_info').insert(user_dict_for_tinydb)


def load_database_to_cache():
    dict_for_cache = {}
    if USE_DATABASE:
...
async def revenue(ctx, username: str, *options: str):
    try:
        ...
        if idcheck:
            ...
            embed = discord.Embed(
                title="Twitch Creator Info - " + display_name + " - (" + username + ")", timestamp=now,
            ...
            if "timeline" in options or "detailed" in options or "piechart" in options or "allgraphs" in options:
                ...
                if "allgraphs" in options:
                    ...
                    await ctx.send(
                        f"{short_url.tinyurl.short('https://quickchart.io/chart?c=' + quote(pie_chart_url))}")
                    return
...
async def compare(ctx, username_one: str, username_two: str, *options: str):
    ...
    if idcheck_one and idcheck_two:
        try:
            ...
            if username_one in cached_results and username_two not in cached_results:
                main_msg = await ctx.send('Data for ' + display_name_two + ' is loading... You will be pinged when the embed is sent!')

            # Check presence of streamer in cache
            if username_one in cached_results:
                streamer_data_one = await retrieve_streamer_data_from_cache(username_one)
            else:
                streamer_data_one = await main_parser(streamer_id_one, display_name_one, main_msg, data2019_one,
                                                      data2020_one, data2021_one)
                # Storage of info in cache
                await store_user_info_in_cache(bio_one, created_one, display_name_one, logo_one, streamer_data_one,
                                               streamer_id_one, username_one)
                await store_user_in_database(username_one)
            ...
            if username_two in cached_results:
                ...
                streamer_data_two = await main_parser(streamer_id_two, display_name_two, main_msg, data2019_two,
                                                      data2020_two, data2021_two)
                # Storage of info in cache
                await store_user_info_in_cache(bio_two, created_two, display_name_two, logo_two, streamer_data_two,
                                               streamer_id_two, username_two)
                await store_user_in_database(username_two)
            ...
            timeline_url = "{options:{title:{display:true,text:'Twitch Creator Data - " + display_name_one + " vs. " + display_name_two + " - Timeline of Gross Total', fontSize:20}},type:'line',data:{labels:" + str(
                LABELS) + ", datasets:[{label:'Gross Total of " + display_name_one + "', data: " + str(
                totaldatagross_one) + ", fill:false,borderColor:'blue'},{label:'Gross Total of " + display_name_two + "', data: " + str(
                totaldatagross_two) + ", fill:false,borderColor:'red'}]}}"

            embed = discord.Embed(
                title=f"Twitch Creator Info - {display_name_one} vs. {display_name_two}", timestamp=now)
            main_data_one = f':moneybag: Gross Total {display_name_one} ' + "`$" + str(
                "{:,}".format(streamer_data_one[0])) + " USD`\n" + ':tv: Ad Total: ' + "`$" + str(
                "{:,}".format(streamer_data_one[1])) + " USD`\n" + ':star: Sub Total: ' + "`$" + str(
                "{:,}".format(streamer_data_one[2])) + " USD`\n" + ':stars: Primers Total: ' + "`$" + str(
                "{:,}".format(streamer_data_one[3])) + " USD`\n" + ':ice_cube: Bits Total: ' + "`$" + str(
                "{:,}".format(streamer_data_one[4])) + " USD`"
            main_data_two = f':moneybag: Gross Total {display_name_two} ' + "`$" + str(
                "{:,}".format(streamer_data_two[0])) + " USD`\n" + ':tv: Ad Total: ' + "`$" + str(
                "{:,}".format(streamer_data_two[1])) + " USD`\n" + ':star: Sub Total: ' + "`$" + str(
                "{:,}".format(streamer_data_two[2])) + " USD`\n" + ':stars: Primers Total: ' + "`$" + str(
                "{:,}".format(streamer_data_two[3])) + " USD`\n" + ':ice_cube: Bits Total: ' + "`$" + str(
                "{:,}".format(streamer_data_two[4])) + " USD`"
...
if check_database_existence():
    cached_results = load_database_to_cache()
else:
    cached_results = {}
